...
function parseInsightRow(row: Record<string, unknown>): CorpusInsight {
  return {
    id: row.id as string,
...
function parseSessionRow(row: Record<string, unknown>): WorkSession {
  return {
    id: row.id as string,
    title: (row.title as string) || '',
    kind: ((row.kind as string) || 'work') as SessionKind,
...
export function registerContentEngineHandlers(): void {
  ipcMain.handle('insights:getAll', async () => {
    const db = getDb()
    const result = await db.execute(
      'SELECT * FROM corpus_insights ORDER BY created_at DESC'
    )
  ...
  ipcMain.handle('insights:create', async (_e, input: CreateInsightInput) => {
    ...
    await db.execute({
      sql: `INSERT INTO corpus_insights
            (id, text, so_what, source, pillar, origin, embedding, dump_id, session_id, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      args: [id, text, soWhat, source, pillar, origin, embedding, dumpId, sessionId, now, now]
    })
  ...
  ipcMain.handle('sessions:create', async (_e, input: CreateSessionInput) => {
    ...
    await db.execute({
      sql: `INSERT INTO sessions
            (id, title, kind, contact_id, project_id, started_at, ended_at, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      args: [id, title, kind, contactId, projectId, startedAt, endedAt, now, now]
    })
