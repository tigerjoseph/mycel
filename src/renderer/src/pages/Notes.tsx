...
export function Notes(): React.JSX.Element {
  ...
  const [fetchedEditNote, setFetchedEditNote] = useState<Note | null>(null)
  ...
  // Open lightbox from Cmd+K search
  useEffect(() => {
    if (activeNoteId) {
  ...
  // If Cmd+K opens a note not yet in the list, fetch a stub so the lightbox can open
  useEffect(() => {
    if (!editingNoteId || editingNote) {
      setFetchedEditNote(null)
      return
    }
    let cancelled = false
    window.mycel.getNote(editingNoteId).then((full) => {
      if (!cancelled && full) setFetchedEditNote(full as Note)
    })
    return () => { cancelled = true }
  }, [editingNoteId, editingNote])

  const lightboxNote = editingNote ?? fetchedEditNote
